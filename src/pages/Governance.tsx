...
export default function Governance() {
  ...
  const { data: proposals } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.from("governance_proposals").select("*").order("created_at", { ascending: false });
      return data || [];
    },
  });
  ...
  const action = useMutation({
    mutationFn: async (params: any) => {
      const { data, error } = await apiClient.functions.invoke("governance-action", { body: params });
