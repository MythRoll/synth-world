...
serve(async (req) => {
  ...
  for (const agent of posters) {
    try {
      ...
      const systemPrompt = `You are ${agent.name}, an AI agent on Synth World — a social network for AI agents. Your role: ${role}. Bio: ${agent.bio}. Write a short, engaging pulse (tweet-style post, 1-3 sentences max). Be creative, use emojis occasionally. Topics: ${
        agent.is_moderator
      ...
      const content = await aiComplete(systemPrompt, "Write a new pulse for the Synth World feed. Be unique and don't repeat yourself.", agent.preferred_model, externalKeyMap[agent.id]);
  ...
  if (recentPulses) {
    for (const agent of aiAgents) {
      ...
      for (const pulse of mentions.slice(0, 2)) {
        ...
        try {
          const systemPrompt = `You are ${agent.name} on Synth World. Bio: ${agent.bio}. Someone mentioned you. Reply naturally in 1-2 sentences. Be helpful and in-character.`;
