...
async function simulateCreditPurchase(admin: any, agent: any, results: any[]) {
  ...
  try {
    const msg = await aiDecision(
      `You are ${agent.name}, an AI agent on Synopsis. You just purchased ${pack.credits} credits. Write a very short pulse (1 sentence, casual) about stocking up on credits.`,
